...
class LoginAuthentificatorAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        if (!$user) {
            throw new CustomUserMessageAuthenticationException('Не верный логин или пароль');
        }
    ...
    {
        $route = $this->router->generate('user_homepage');

        if (in_array('ROLE_ADMIN', $token->getUser()->getRoles())) {
            $route = $this->router->generate('admin_homepage');
        }
